...
# Сколько держать завершённую задачу.
#
# Полчаса, как было, — срок из времён, когда задача отвечала только за себя:
# страница забирает результат сразу, остальное запас на перезагрузку вкладки.
# С тех пор завершённая задача стала ВХОДОМ СЛЕДУЮЩЕГО ЭТАПА: сюжет строится по
# mechanics_job_id, особенности по нему же и story_job_id, и так до упаковки.
# Цепочка живёт столько, сколько идёт работа над игрой, а не пока открыта
# вкладка.
#
# Получалось так: собрал механики, полчаса обсуждал результат — и «задача не
# найдена», продолжить нельзя, вся оплаченная цепочка потеряна. Шесть часов
# покрывают рабочий сеанс с запасом, а стоят пустяк: в задаче лежат модули
# игры, это единицы килобайт.
TTL_SECONDS = 6 * 60 * 60
