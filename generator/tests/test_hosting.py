...
# --------------------------------------------------------------------------
# Срок ожидания оценки по линзам
# --------------------------------------------------------------------------

def test_ждём_дольше_бюджета_оценщика(monkeypatch):
    """Оценка иногда не укладывалась и в 360 с. Сдавшийся раньше генератор
    показывает ошибку по несуществующему поводу, пока работа идёт."""
    assert настройка(monkeypatch).lens_timeout >= 1000


def test_срок_приходит_от_самого_оценщика(monkeypatch):
    """Две константы в разных сервисах расходятся молча, поэтому главный
    источник — поле budget_seconds в ответе оценщика, а своё значение
    запасное."""
    from agents import lens_review
    monkeypatch.setattr(настройка(monkeypatch), "lens_timeout", 1100)
    assert lens_review.wait_for({"budget_seconds": 1000}) == 1000 + lens_review.WAIT_SLACK
