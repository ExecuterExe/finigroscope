...
# Слова, которыми отрицают ДЕЙСТВИЕ. Нужны, чтобы не поймать выполненное
# требование как его нарушение: правила игры без выбывания ОБЯЗАНЫ сказать
# «никто не выбывает», и это ровно то, чего от них хотели.
#
# «без» сюда НЕ входит, хотя напрашивается: оно относится к существительному, а
# не к глаголу. Во фразе «игрок, оставшийся без жетонов, выбывает из партии»
# оно не отрицает выбывание — наоборот, называет его причину. С «без» в списке
# настоящее выбывание переставало замечаться.
NEGATIONS = ("не ", "нет ", "никто", "никогда", "ни один")

# Насколько близко перед словом должно стоять отрицание. Коротко намеренно:
# «никто не выбывает» — отрицание вплотную. Чем шире окно, тем чаще под него
# попадает отрицание из соседнего предложения.
NEGATION_WINDOW = 25


def _forbidden_hit(text, words):
    """Первое НЕ отрицаемое вхождение запретного слова. None — чисто.

    Отрицание проверяется намеренно: «никто не выбывает» содержит «выбыва», и
    без этой проверки правила бракуются за то, что выполнили требование автора
    буквально. Так и происходило: модуль правил не проходил ни одной из трёх
    попыток подряд.
    """
    for word in words:
        for match in re.finditer(re.escape(word), text):
            перед = text[max(0, match.start() - NEGATION_WINDOW):match.start()]
            # Знак препинания обрывает связь: «…без жетонов, выбывает» — это
            # уже другая часть предложения, и отрицание из первой к глаголу не
            # относится. Поэтому ищем отрицание только ПОСЛЕ последней запятой
            # или точки.
            for знак in ",.;:!?":
                перед = перед.rsplit(знак, 1)[-1]
            if any(n in перед for n in NEGATIONS):
                continue
            return word
    return None
...
def validate(data, params, modules, components):
    ...
    for variant in variants:
        ...
        for name, row in allowed.items():
            # Число рядом с компонентом — НЕ обязательно заявка об общем
            # количестве. Правила законно раздают части: «раздайте каждому по
            # 10 карт» при двадцати картах на двоих — верно, а не ошибка.
            #
            # Первая версия сравнивала любое такое число с итогом и браковала
            # модуль. На живом прогоне правила не прошли ни одной из трёх
            # попыток подряд с «названо 10, а посчитано 20» — то есть за
            # правильную раздачу.
            #
            # Настоящий дефект тут один: правила требуют БОЛЬШЕ, чем есть в
            # коробке. Меньше — обычное дело.
            total = row.get("quantity") or 0
            слишком = sorted(n for n in _mentioned_quantities(text, name)
                             if n > total)
            if слишком:
                # Предупреждение, а не брак: это разбор прозы регулярным
                # выражением, а такой разбор не вправе отменять оплаченную
                # работу. Автор увидит замечание и решит сам.
                warnings.append(
                    "%s: для «%s» в правилах названо %s — больше, чем в коробке "
                    "(%s). Проверьте, не требуется ли больше, чем посчитано."
                    % (label, name, ", ".join(map(str, слишком)), total))
