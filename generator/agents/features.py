...
Заполняй его, ТОЛЬКО если приём физически не работает без предметов сверх
базового набора — например требует отдельной колоды ролей. Каждый лишний
предмет удорожает коробку, поэтому «на всякий случай» просить не надо.

Если всё же заполняешь, элемент выглядит так:
...
PARAM_KEYS = ("purpose", "player_count", "adaptation", "disabilities",
              ...
              # components нужны не для выбора приёмов, а для заявок на
              # дополнительные предметы: называть в них разрешено ТОЛЬКО то, что
              # автор выбрал, — а не видя списка, модель может лишь угадывать.
              "components",
              "custom")
