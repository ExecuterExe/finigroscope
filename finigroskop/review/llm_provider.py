...
# Провайдер, когда не выбрано ничего: ни в интерфейсе, ни в .env.
#
# Раньше здесь был «null» — ИИ выключен, и это было честно, пока сервис работал
# сам по себе: не настроил ключ — агенты молчат, а не падают. Но ФинИгроСкоп
# перестал быть отдельным сервисом: «Генератор игр» присылает ему модули на
# оценку по линзам, и ключ для этого один на оба (в render.yaml он и заведён
# общей группой). Умолчание «выключено» означало, что после чистой установки
# первый же запрос от соседа получал отказ, пока человек не сходит в настройки и
# не выберет провайдера руками.
#
# Ключа это не подменяет: без OPENROUTER_API_KEY провайдер честно скажет, чего
# не хватает, и назовёт переменную. Разница в том, что теперь он говорит
# «нет ключа», а не «ИИ выключен» — второе не подсказывает вообще ничего.
...
def resolve_provider_name(explicit: str = None) -> tuple[str, str]:
    ...
    return DEFAULT_PROVIDER, "по умолчанию"
