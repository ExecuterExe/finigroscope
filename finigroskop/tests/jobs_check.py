...
# Ориентир длительности — по нему экран решает, пора ли сказать «дольше
# обычного». Шаг без ориентира молчал бы вечно, и на пятой минуте спокойная
# строка читалась бы как поломка.
checks["П9: у каждого шага есть ориентир длительности"] = all(
    jobs.step_usual(s) > 0 for s in jobs.STEP_TITLES)
checks["П9: у неизвестного шага ориентира нет"] = jobs.step_usual("???") == 0
# Ориентиры взяты из живых прогонов; порядок величин обязан им соответствовать,
# иначе «дольше обычного» появится либо сразу, либо никогда.
checks["П9: линзы дольше механик"] = (
    jobs.step_usual("lenses") > jobs.step_usual("mirror"))
checks["П9: вердикты диагноста дольше триажа"] = (
    jobs.step_usual("diagnost_findings") > jobs.step_usual("diagnost_triage"))
