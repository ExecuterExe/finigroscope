...
STEP_TITLES = {
    ...
    "local_run": "Локальный прогон скелета",
...
STEP_HINTS = {
    ...
    "local_run": "2000 партий на конфигурацию — обычно секунды, предел 2 минуты",
...
# Сколько шаг обычно идёт, В СЕКУНДАХ. Те же числа, что в подсказках выше, но
# машинно-читаемые: по ним экран решает, пора ли сказать «дольше обычного».
# Взяты из живых прогонов (диагност — 126 и 352 с, линзы — 564 с), остальные
# округлены вверх. Это ориентир для человека, а не таймаут: превышение ничего
# не обрывает, оно только меняет строку на экране.
STEP_USUAL_SECONDS = {
    "mirror": 60,
    "extraction": 60,
    "gate2": 60,
    "skeleton": 180,
    # Прогон — единственный шаг, где считает не модель, а процессор: секунды
    # на конфигурацию. Ориентир взят с запасом на медленную машину.
    "local_run": 30,
    "balance": 45,
    "diagnost_triage": 150,
    "diagnost_findings": 360,
    "lenses": 570,
    "synthesis": 60,
    "redesign": 60,
}
...
def step_usual(step: str) -> int:
    """Обычная длительность шага в секундах. 0 — ориентира нет, молчим."""
    return STEP_USUAL_SECONDS.get(step, 0)
